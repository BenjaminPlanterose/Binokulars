# Rscript --vanilla $MY_PATH/binomL.R --target $target_file --epiread_chr_dir $input_dir --bin_length $bin_length 
# --N_iter $N_iter --seed $SEED --output_dir $output_dir --nCores $nCores
...
permutation_test <- function(X, breaks, N_iter)
{
  ...
  if(logL == Inf) # Problems between Fwd and Rv strands
  {
    logL = 0
  }
...
Start = as.integer(sapply(strsplit(pos, split = '-'), function(x) x[1])) - 500 # Add flanks
End = as.integer(sapply(strsplit(pos, split = '-'), function(x) x[2])) + 500 # Add flanks
